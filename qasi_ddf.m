...
while loop_num < NUM_LOOPS
    ...
    for a = 1:BLUE_NUM
        [x_hat, P] = get_estimate(x_hats, Ps, NUM_STATES, a);
        ...
        % Filter Measurements
        [x_hat, P] = filter_dvl(x_hat,P,x_gt, w, w_perceived, NUM_AGENTS, NUM_STATES, a);
        [x_hat, P] = filter_sonar(x_hat, P, x_gt, w, w_perceived, NUM_AGENTS, NUM_STATES, PROB_DETECTION, SONAR_RANGE);
        
        [x_hats, Ps] = set_estimate(x_hats, Ps, NUM_STATES, x_hat, P, a);
    end
    ...
    if schedule_count== 9 % Global Measurements
        for a = 1:BLUE_NUM
            [x_hat, P] = get_estimate(x_hats, Ps, NUM_STATES, a);
            [x_hat, P] = filter_gps(x_hat,P,x_gt, w_gps, w_gps_perceived, NUM_AGENTS, NUM_STATES, 0);
            [x_hats, Ps] = set_estimate(x_hats, Ps, NUM_STATES, x_hat, P, a);
        end
    else % Broadcast states
       for a = 1:BLUE_NUM
           if schedule_count== 4*a + 9
               [x_hat1, P1] = get_estimate(x_hats, Ps, NUM_STATES, a);
               for a2 = 1:BLUE_NUM
                  if a == a2
                      continue
                  else
                      [x_hat2, P2] = get_estimate(x_hats, Ps, NUM_STATES, a2);
                      [c_hat, C] = covariance_intersect(x_hat1, P1, x_hat2, P2);
                      [x_hats, Ps] = set_estimate(x_hats,Ps, NUM_STATES, c_hat, C, a2);
                  end
              end
              break 
           end
    ...
    % Store Estimates
    [x_hat, P] = get_estimate(x_hats, Ps, NUM_STATES, AGENT_TO_PLOT);
...
%     elem_history = [];
%     for k = 1:NUM_LOOPS
%         elem_history = [elem_history, P_history(i,1+(k-1)*NUM_STATES)];
%     end
